...
export interface CompetitorEntry {
  /** Project-relative year the competitor enters the market */
  entryYear: number;
  ...
  type: 'generic' | 'biosimilar' | 'nextGen' | 'classCompetitor' | 'biosimilarDelayed';
  /** Optional name (from real pipeline data when available) */
  name?: string;
  /** Optional mechanism of action (used for MOA clustering) */
  moa?: string;
  /** Whether this competitor requires the same biomarker to compete */
...
export interface CompetitiveDynamicsResult {
  ...
  priceImpact: number;
  /** Year of peak erosion (project-relative) */
  peakErosionYear: number;
  /** Confidence level in the estimate */
  confidence: 'high' | 'moderate' | 'low';
  /** Calibration source used (template vs real pipeline data) */
...
// elsewhere in the engine. Linear ramps systematically underestimate
// early competitor impact and overestimate late impact.
function sCurveRamp(t: number): number {
  const clamped = Math.max(0, Math.min(1, t));
  return 3 * clamped * clamped - 2 * clamped * clamped * clamped;
}

// ─── Helper: Select a primary MOA from the input ───
// Derives a coarse MOA bucket from the modality so we can detect same-MOA
// clustering when real pipeline data is provided.
 ...
 * @param pipelineData - Optional real pipeline intelligence to override template
...
export function calculateCompetitiveDynamics(
  ...
  // so launchYear must also be project-relative to match.
  const launchYear = Math.ceil(baseResult.yearsToMarket);

  // ─── Upgrade 1: Load TA-specific calibration ───
  ...
  // ─── Upgrade 2: Modality-aware entry lag ───
  ...
  // Effective entry lag = base lag × modality multiplier + position adjustment + reg delay
  const effectiveEntryLag = Math.max(
    -3, // allow pre-launch competitors in crowded markets
    intensity.avgEntryLagYears * modalityLagMult + positionMod.entryLagAdjustment + regDelay,
  );

  // ─── Build competitor timeline ───
  // Either from real pipeline data (if provided) or calibrated templates.
  ...
  if (pipelineData?.knownCompetitors && pipelineData.knownCompetitors.length > 0) {
    ...
    for (const comp of pipelineData.knownCompetitors) {
      // Convert absolute expected approval year (if provided) to project-relative
      const relativeEntry = comp.expectedApprovalYear != null
        ? comp.expectedApprovalYear - currentYear
        : launchYear + intensity.avgEntryLagYears;

      // ─── Upgrade 8: MOA clustering ───
      const sameMOA = comp.moa && comp.moa === primaryMOA;
      const moaMultiplier = sameMOA ? 1.3 : 0.85; // Same-MOA takes more share

      // ─── Upgrade 7: Biomarker segmentation ───
      // If asset is biomarker-selected and competitor also requires biomarker,
      // they compete directly. Otherwise, biomarker-gated competitors only
      // erode a fraction of the addressable market.
      let biomarkerMultiplier = 1.0;
      if (comp.biomarkerRequired && input.biomarkerStatus !== 'validated') {
        biomarkerMultiplier = 0.4; // Only erodes the ~40% biomarker-positive segment
      }
    ...
    // Add any additional unnamed competitors from pipeline data
    ...
    // ─── Fallback: calibrated template ───
    // Stagger competitor entries to reflect real-world sequential entry patterns.
    for (let i = 0; i < effectiveCompetitorCount; i++) {
      const entryOffset = effectiveEntryLag + i * 1.2; // ~1.2 year gap between entrants
      const isPreLaunch = launchYear + entryOffset < launchYear;

      // Vary share impact: first competitor takes the most, subsequent take less
      // This reflects diminishing returns as the market gets more crowded.
      ...
      competitorTimeline.push({
        ...
        rampYears: isPreLaunch ? 0.5 : 2, // pre-launch competitors ramp faster
        type: i === 0 ? 'classCompetitor' : 'nextGen',
      });
    }
  }

  // ─── Year-by-year calculation with S-curve ramp + price erosion ───
  ...
  let peakErosionYear = launchYear;
  ...
  for (let i = 0; i < cashFlows.length; i++) {
    ...
    for (const competitor of competitorTimeline) {
      if (year >= competitor.entryYear) {
        ...
        // S-curve ramp (cubic easing) replaces the previous linear ramp
    ...
    // Apply first-mover shield in the early post-launch years
    ...
    // ─── Upgrade 5: Price erosion ───
    // Each active competitor drives per-year price pressure. This is on top
    // of volume erosion — competitors steal share AND drive pricing down.
    ...
    // Track separate volume and price impacts for decomposition
    ...
    if (yearCombinedErosion > peakShareErosion + peakPriceErosion) {
      peakErosionYear = year;
    }
  ...
  // Build narrative with specific competitor names if available
  const namedCompetitors = competitorTimeline.filter((c) => c.name).map((c) => c.name);
  const competitorDetail = namedCompetitors.length > 0
    ? ` Named competitors include: ${namedCompetitors.slice(0, 3).join(', ')}${namedCompetitors.length > 3 ? ` and ${namedCompetitors.length - 3} more` : ''}.`
    : '';

  // Regulatory protection narrative
  const regProtection = regDelay > 0
    ? ` Regulatory exclusivity (${input.regulatoryDesignations?.orphan ? 'orphan, ' : ''}${input.regulatoryDesignations?.breakthrough ? 'breakthrough, ' : ''}${input.regulatoryDesignations?.fastTrack ? 'fast track, ' : ''}${input.regulatoryDesignations?.prime ? 'PRIME' : ''}) provides +${regDelay.toFixed(1)} years of effective protection.`.replace(/, \)/, ')')
    : '';
  ...
  if (combinedErosion < 0.20) {
    narrative = `Favorable competitive outlook with ${competitorCount} anticipated entrant${competitorCount !== 1 ? 's' : ''}. ` +
      `Peak erosion reaches ${totalErosionPct}% (${volumeErosionPct}% volume, ${priceErosionPct}% price) in year ${peakErosionYear}, ` +
      `reflecting strong positioning${regDelay > 0 ? ' and regulatory protection' : ''}.${regProtection}${competitorDetail} ` +
      `Total lifetime revenue impact: $${revenueImpact.toFixed(0)}M ($${totalVolumeImpact.toFixed(0)}M volume + $${totalPriceImpact.toFixed(0)}M price).`;
    ...
    narrative = `Moderate competitive pressure with ${competitorCount} anticipated entrant${competitorCount !== 1 ? 's' : ''}. ` +
      ...
      `Peak erosion of ${totalErosionPct}% (${volumeErosionPct}% volume share lost, ${priceErosionPct}% price erosion) in year ${peakErosionYear} ` +
      `reduces lifetime revenue by $${revenueImpact.toFixed(0)}M. Volume impact: $${totalVolumeImpact.toFixed(0)}M; price impact: $${totalPriceImpact.toFixed(0)}M.${regProtection}${competitorDetail} ` +
      `Differentiation, pricing strategy, and launch sequencing are critical levers.`;
  } else {
    narrative = `Significant competitive headwinds with ${competitorCount} anticipated entrant${competitorCount !== 1 ? 's' : ''}. ` +
      `First competition expected ${firstCompetitorText}. ` +
      `Peak erosion of ${totalErosionPct}% (${volumeErosionPct}% volume + ${priceErosionPct}% price compression) in year ${peakErosionYear} ` +
      `substantially impacts commercial potential, with $${revenueImpact.toFixed(0)}M in lifetime revenue at risk ` +
      `(${totalVolumeImpact.toFixed(0)}M volume + ${totalPriceImpact.toFixed(0)}M price).${regProtection}${competitorDetail} ` +
      `Consider accelerated timelines, best-in-class differentiation, and pricing resilience strategies.`;
  }
