...
export function buildIndicativeTermSheet(input: {
  ...
  /** "X appear in 40% of comparable deals" or, with no precedent, "precedent share for X not measurable in the comparable set". */
  const shareClause = (key: string, what: string) => { const s = clauseShare(key); return s == null ? `precedent share for ${what} not measurable in the comparable set` : `${what} appear in ${Math.round(s * 100)}% of comparable deals`; };
  const TERRITORY_LABEL: Record<string, string> = { global: 'Worldwide', us: 'United States', ex_us: 'Worldwide excluding the United States', ex_china: 'Worldwide excluding Greater China', japan: 'Japan', china: 'Greater China', europe: 'Europe', asia_pacific: 'Asia-Pacific' };
  ...
  lines.push({ term: 'Structure', position: isMA ? 'Acquisition of the asset (or the company) with contingent consideration' : isOption ? 'Option to an exclusive licence, with an evaluation period' : isCodev ? 'Co-development and commercialisation licence with a profit-share election' : 'Exclusive licence to develop and commercialise', basis: `Structure you are preparing for at intake${isMA ? '' : (clauseShare('exclusive') ?? clauseShare('exclusivity')) != null ? `; ${Math.round(((clauseShare('exclusive') ?? clauseShare('exclusivity')) as number) * 100)}% of comparable licences are exclusive` : ''}` });
  lines.push({ term: 'Territory', position: chinaLicensed ? `Worldwide excluding Greater China (already licensed${chinaPartner ? ` to ${chinaPartner}` : ''})` : retainChina ? 'Worldwide excluding Greater China (retained)' : /global|world/i.test(territory) ? 'Worldwide' : (TERRITORY_LABEL[territory] ?? territory.replace(/_/g, ' ')), basis: chinaLicensed ? 'Greater China rights are out under an existing licence (intake); the buyer takes the rest of the world' : retainChina ? 'No buyer on the list is Asia-based; Greater China is a second transaction' : 'Territory on offer at intake' });
  if (isOption) lines.push({ term: 'Option fee', position: `${fmt(optionFee)}, creditable against the upfront on exercise`, basis: (prefNum('optionFeeM') ? 'The fee you said you would accept at intake' : 'Sized to the buyer\'s diligence cost (decision lever)') + '; ' + shareClause('option_fee', 'option fees'), floor: `${fmt(Math.round(decision.floor.upfrontM * 0.2))}` });
  ...
  if (!isMA) lines.push({ term: 'Sales milestones', position: b === 'phase_3' ? 'Stepped at $500M and $1B net sales' : 'Stepped at $500M and $1B net sales, in addition to the schedule', basis: shareClause('sales_milestones', 'sales milestones') });
  ...
  lines.push({ term: 'Diligence', position: 'Commercially reasonable efforts with objective diligence milestones (first patient dosed, filing) and reversion on failure', basis: shareClause('diligence', 'diligence obligations') });
