    ...
    for (const phase of PHASES) {
      for (const territory of TERRITORIES) {
        try {
          ...
          const metaDescription = `${phase.label} ${ta.label.toLowerCase()} licensing deals${territoryLabel ? ` in ${territory.label}` : ''}: ${formatCurrency(result.terms.upfront.median)} median upfront, ${formatCurrency(result.terms.totalDealValue.median)} total value, ${result.tieredRoyalties.base.low}–${result.tieredRoyalties.base.high}% royalty. From 3,500+ verified transactions.`;
