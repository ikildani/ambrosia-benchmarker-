 ...
 * (if enabled) and materialises the accuracy rollups. Never throws; returns a
 * report and logs exactly one line with counts.
 */
...
export async function runOutcomePhase(supabase: SupabaseClient, opts: OutcomePhaseOptions): Promise<OutcomePhaseReport> {
  ...
  console.log(
    ...
    (nightly ? ` | nightly: radar=${radar ? `${radar.inserted}/${radar.candidates}${radar.enabled ? '' : ' (off)'}` : '-'} rollups=${rollups ? `${rollups.cells} cells from ${rollups.inputRows} rows` : '-'}` : '') +
    ` | ${ms}ms${errors.length ? ` | errors=${errors.length}: ${errors.slice(0, 3).join('; ')}` : ''}`,
  );
  return { resolver, radar, rollups, nightly, ms, errors };
}
