...
export function renderPipelineMapPage(data: PDFReportData, meta: ReportMeta): string {
  ...
  if (!map) {
    ...
    <div class="report-page">
      ...
      ${head}
      ${emptyState('No pipeline map for this indication', 'Fewer than three active industry or academic programs matched the indication in the trial registry, so a bucket-by-phase map would not be meaningful. The comparable set and buyer map still apply.')}
    ...
    <div class="report-page">
      ...
      <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 12px; margin-top: 12px;">
        ...
        <div class="card-sm" style="border-left: 3px solid ${crowdingColor}; display: flex; flex-direction: column; justify-content: center;">
          ...
          <div style="font-size: 8px; color: ${COLORS.gray500}; margin-top: 4px; line-height: 1.45;">Sixty points saturate at ten same-mechanism programs at or ahead of our phase; forty scale with that bucket's share of the whole map.</div>
        </div>
      </div>
