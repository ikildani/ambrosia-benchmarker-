...
export function renderPartnersPage(data: PDFReportData, meta: ReportMeta): string {
    ...
    return `
      <div class="report-page">
        ...
        <div class="card" style="text-align: center; padding: 40px;">
          <div style="font-size: 14px; color: ${COLORS.gray400}; margin-bottom: 8px;">Partner matching not available for this report.</div>
          <div style="font-size: 11px; color: ${COLORS.gray400};">Visit calculator.ambrosiaventures.co to run the full partner analysis.</div>
        </div>
  ...
  return `
    <div class="report-page">
      ...
      <div class="section-title-lg">Partner Landscape</div>
      <p style="font-size: 11px; color: ${COLORS.gray500}; margin-bottom: 16px;">
        ...
        ${topPartners.map((p, i) => {
          ...
          <div class="card">
            <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 8px;">
              <div>
                <div style="font-size: 12px; font-weight: 600; color: ${COLORS.navy};">${escapeHtml(p.company_name)}</div>
                ${p.hq_country ? `<div style="font-size: 9px; color: ${COLORS.gray400};">${escapeHtml(p.hq_country)}</div>` : ''}
              </div>
              <div style="text-align: right;">
                <div style="font-size: 18px; font-weight: 700; color: ${scoreColor};">${p.match_score}%</div>
                <div style="font-size: 8px; color: ${COLORS.gray400};">MATCH</div>
              </div>
            </div>
            <!-- Score bar -->
            ...
            <div style="font-size: 9px; color: ${COLORS.gray500};">${p.deals_last_12mo} deals in last 12 months</div>
          </div>
          `;
        }).join('')}
      </div>
          ...
          <tbody>
            ${topPartners.map(p => {
              ...
              <tr>
                ...
                <td style="text-align: center; background: ${cellBg}; font-weight: 600; color: ${COLORS.navy};">${p.match_score}%</td>
                <td style="text-align: center;">${p.deals_last_12mo}</td>
                <td style="text-align: center; font-size: 10px;">${escapeHtml(p.hq_country || 'N/A')}</td>
                <td style="text-align: center; font-size: 10px;">${p.match_reasons[0] ? escapeHtml(p.match_reasons[0].reason) : 'N/A'}</td>
              </tr>
              `;
            }).join('')}
          </tbody>
