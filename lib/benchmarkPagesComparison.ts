...
function buildModalityVsModalityPage(def: ModalityPairDef): BenchmarkPageData {
  ...
  const metaDescription = `Compare ${labelA.toLowerCase()} vs ${labelB.toLowerCase()} deal terms in ${taLabel.toLowerCase()}: ${labelA} median upfront ${formatCurrency(rA.terms.upfront.median)} vs ${labelB} ${formatCurrency(rB.terms.upfront.median)}. Side-by-side benchmarks from ${DEAL_STATS.TOTAL_DEALS} deals.`;
...
function buildTAVsTAPage(def: TAPairDef): BenchmarkPageData {
  ...
  const metaDescription = `Compare ${labelA.toLowerCase()} vs ${labelB.toLowerCase()} licensing deal terms: ${labelA} median upfront ${formatCurrency(rA.terms.upfront.median)} vs ${labelB} ${formatCurrency(rB.terms.upfront.median)}. Side-by-side TA benchmarks from ${DEAL_STATS.TOTAL_DEALS} deals.`;
