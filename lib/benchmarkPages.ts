...
import { getCompanyBenchmarkPages } from './benchmarkPagesCompany';
import { getIndicationBenchmarkPages } from './benchmarkPagesIndication';
import { getComparisonBenchmarkPages } from './benchmarkPagesComparison';
...
function getMergedPages(): BenchmarkPageData[] {
  ...
  const company = getCompanyBenchmarkPages() as unknown as BenchmarkPageData[];
  const indication = getIndicationBenchmarkPages();
  const comparison = getComparisonBenchmarkPages();
  const allGenerated = [...generated, ...company, ...indication, ...comparison];
  const handWrittenSlugs = new Set(BENCHMARK_PAGES.map((p) => p.slug));
  const seen = new Set<string>(handWrittenSlugs);
  const uniqueGenerated = allGenerated.filter((p) => {
...
// ── Supabase-backed dynamic page resolution ──────────────────────────────────
// Closes the gap between seo-page-expand cron (stores to seo_generated_pages)
// and the benchmark page template. Pages generated by the cron are served
// without anyone manually adding them to code.

import { createServiceClient } from '@/lib/supabase/server';

let _dbSlugsCache: string[] | null = null;
let _dbSlugsCacheTime = 0;
const DB_CACHE_TTL = 5 * 60 * 1000; // 5 minutes

export async function getBenchmarkBySlugWithDb(slug: string): Promise<BenchmarkPageData | undefined> {
  const codePage = getBenchmarkBySlug(slug);
  ...
  try {
    ...
    if (data?.page_data) {
      return data.page_data as unknown as BenchmarkPageData;
    }
  } catch {
    // Supabase unavailable — fall back to code-only
  }
  return undefined;
}

export async function getAllBenchmarkSlugsWithDb(): Promise<string[]> {
  const codeSlugs = getAllBenchmarkSlugs();

  const now = Date.now();
  if (_dbSlugsCache && now - _dbSlugsCacheTime < DB_CACHE_TTL) {
    const codeSet = new Set(codeSlugs);
    const dbOnly = _dbSlugsCache.filter(s => !codeSet.has(s));
    return [...codeSlugs, ...dbOnly];
  }
  ...
  try {
    ...
    if (data) {
      _dbSlugsCache = data.map(r => r.slug);
      _dbSlugsCacheTime = now;
      const codeSet = new Set(codeSlugs);
      const dbOnly = _dbSlugsCache.filter(s => !codeSet.has(s));
      return [...codeSlugs, ...dbOnly];
    }
