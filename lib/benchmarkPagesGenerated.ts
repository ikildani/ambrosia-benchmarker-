...
function generateTAModalityPage(
  ...
  const metaDescription = `Comprehensive ${modLabel.toLowerCase()} licensing and acquisition deal benchmarks for ${taLabel.toLowerCase()}. Median upfront ${formatCurrency(r.terms.upfront.median)}, total deal value ${formatCurrency(r.terms.totalDealValue.median)}, and royalty rates from ${DEAL_STATS.TOTAL_DEALS} verified deals.`;

  const contextParagraphs = [
    `${modLabel} therapies in ${taLabel.toLowerCase()} have seen significant M&A and licensing activity in recent years. Based on analysis of ${DEAL_STATS.TOTAL_DEALS} verified transactions, the median upfront payment for a Phase 2 ${modLabel.toLowerCase()} asset in ${taLabel.toLowerCase()} is ${formatCurrency(r.terms.upfront.median)}, with total deal values reaching ${formatCurrency(r.terms.totalDealValue.median)}. Development milestones average ${formatCurrency(r.terms.devMilestones.median)}, while regulatory milestones contribute ${formatCurrency(r.terms.regMilestones.median)}.`,
...
function generateTAPhasePage(
  ...
  const metaDescription = `${phaseLabel} ${taLabel.toLowerCase()} licensing deal benchmarks: median upfront ${formatCurrency(r.terms.upfront.median)}, total deal value ${formatCurrency(r.terms.totalDealValue.median)}, and royalty rates. Data from ${DEAL_STATS.TOTAL_DEALS} verified transactions.`;
