...
export async function GET(request: NextRequest) {
  ...
  if (only) {
    const a = [MFN, RESOURCE, HKEX_BACKFILL, ...ADAPTERS].find(x => x.key === only);
    if (!a) return NextResponse.json({ error: `unknown adapter ${only}`, adapters: [MFN, RESOURCE, HKEX_BACKFILL, ...ADAPTERS].map(x => x.key) }, { status: 400 });
