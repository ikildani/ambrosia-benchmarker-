# Outreach Email Templates

## Template 1: BD Consultants & Advisory Firms (Highest Priority)

**Subject**: Instant biopharma deal benchmarks — 600+ real transactions

Hi [Name],

I saw [Firm]'s work on [specific: "the 2026 valuation guide" / "the Gilead licensing advisory" / etc.]. We built something your team might find useful.

**Ambrosia Ventures Deal Calculator** gives you instant benchmarks for biopharma licensing deal terms — upfront payments, milestone structures, royalty rates — powered by 600+ real transactions across 8 therapeutic areas.

What makes it different:
- **rNPV engine** with Monte Carlo simulation (10,000 iterations)
- **Comparable deal analysis** from our curated database
- **AI-generated negotiation playbooks** and deal memos
- **Scenario planner** with 28 stress-test scenarios and defensive analysis

It's free to try: https://calculator.ambrosiaventures.co/calculator

We're exploring partnerships with advisory firms. Would love 15 minutes to show you how [Firm] could use it — or white-label it for client work.

Best,
[Your name]
Ambrosia Ventures

---

## Template 2: Mid-Size Biotech BD Teams

**Subject**: How [Company] BD team can benchmark deals in 30 seconds

Hi [Name],

Saw that [Company] recently [specific: "signed the ADC licensing deal with X" / "is evaluating partnerships in CNS" / etc.].

We built a tool that gives BD teams instant deal term benchmarks from 600+ real biopharma transactions — so you can walk into negotiations with data-backed ranges instead of spending weeks on manual comps.

**For a Phase 2 [relevant modality] in [relevant TA]**, the calculator instantly shows:
- Upfront payment range (25th-75th percentile)
- Milestone structure breakdown (dev/reg/commercial)
- Royalty rate benchmarks
- rNPV analysis with Monte Carlo simulation
- Top 15 comparable deals

Try it free: https://calculator.ambrosiaventures.co/calculator

Would a 15-minute demo be useful for your BD team?

Best,
[Your name]

---

## Template 3: Life Sciences VCs

**Subject**: Deal term benchmarks for portfolio diligence — 600+ real transactions

Hi [Name],

When evaluating licensing terms for portfolio companies, having real benchmark data matters. We built a tool that gives instant deal term analysis from 600+ biopharma transactions.

**Ambrosia Ventures Deal Calculator** helps investors:
- Benchmark proposed deal terms against real market data
- Run rNPV analysis with Monte Carlo simulation
- Stress-test valuations with 28 scenario templates
- Compare across 8 therapeutic areas and all major modalities

Free to try: https://calculator.ambrosiaventures.co/calculator

Worth a look next time you're evaluating a licensing term sheet.

Best,
[Your name]

---

## Template 4: Deal Data Platforms (Partnership)

**Subject**: Partnership opportunity — complementary deal intelligence tools

Hi [Name],

I'm the founder of Ambrosia Ventures. We've built a deal benchmarking calculator that provides instant rNPV analysis and deal term estimates from 600+ curated biopharma transactions.

I think there's a strong partnership opportunity between [Platform] and our calculator:
- **Your strength**: comprehensive deal database / partnering platform
- **Our strength**: instant analytical engine (rNPV, Monte Carlo, scenario planning, AI deal memos)

Potential: co-marketing, data integration, or referral partnership.

Would you be open to a 20-minute conversation?

Best,
[Your name]
https://calculator.ambrosiaventures.co

---

## Template 5: Journalists / Podcasts

**Subject**: New free tool for biopharma deal benchmarking — story/guest pitch

Hi [Name],

I built a free tool that lets biopharma professionals instantly benchmark licensing deal terms — upfront payments, milestones, royalties — from 600+ real transactions across 8 therapeutic areas.

**What makes it newsworthy:**
- First free tool with rNPV + Monte Carlo simulation for deal benchmarking
- Covers all major modalities (ADCs, CAR-T, gene therapy, bispecifics, etc.)
- AI-generated deal memos and negotiation playbooks
- 120+ company competitive intelligence profiles

Happy to provide:
- Demo / walkthrough for a story
- Guest article on deal benchmarking trends
- Data insights (e.g., "How ADC deal terms have evolved 2020-2026")

Try it: https://calculator.ambrosiaventures.co

Best,
[Your name]

---

## Template 6: Conferences / Associations

**Subject**: Sponsorship / speaking opportunity — biopharma deal benchmarking

Hi [Name],

I'm the founder of Ambrosia Ventures. We've built the first free biopharma deal benchmarking tool with rNPV analysis and Monte Carlo simulation, used by BD professionals across 8 therapeutic areas.

I'd love to discuss:
- **Speaking**: "Data-Driven Deal Benchmarking: What 600+ Transactions Tell Us About 2026 Deal Terms"
- **Sponsorship**: booth or tool demo at [Event Name]
- **Content partnership**: co-branded deal term analysis for your members

Our tool: https://calculator.ambrosiaventures.co

Would this be of interest for [upcoming event]?

Best,
[Your name]

---

## LinkedIn DM Template (Short)

Hi [Name] — I noticed your work in [biopharma BD / licensing / deal advisory]. We built a free tool that gives instant deal term benchmarks from 600+ real biopharma transactions (rNPV, Monte Carlo, AI deal memos). Would love your feedback: calculator.ambrosiaventures.co

---

## X/Twitter Post Ideas

1. "We analyzed 600+ biopharma licensing deals. Here's what Phase 2 oncology ADC upfronts look like in 2026: [screenshot of calculator results]. Try it free: calculator.ambrosiaventures.co"

2. "Building an rNPV model for a licensing deal? We built a free Monte Carlo simulator with 10,000 iterations, 28 scenario templates, and AI deal memos. calculator.ambrosiaventures.co"

3. "BD teams: stop spending 2 weeks on manual deal comps. Get instant benchmarks from 600+ real transactions in 30 seconds. Free: calculator.ambrosiaventures.co"
