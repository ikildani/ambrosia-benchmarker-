 ...
 *   6. Append result to /tmp/apr20-outreach/final.jsonl
 *
 * Usage:
...
function loadApolloMap(): Map<string, ApolloRecord> {
  const lines = readFileSync('/tmp/apr20-outreach/apollo_results.jsonl', 'utf-8')
    .split('\n').filter(Boolean);
...
async function main() {
  const contacts: SrcContact[] = JSON.parse(readFileSync('/tmp/apr20-outreach/contacts_enriched.json', 'utf-8'));
  const apollo = loadApolloMap();
  const outPath = '/tmp/apr20-outreach/final.jsonl';
