 ...
 *   6. Append result to /tmp/apr14-outreach/final.jsonl
 *
 * Usage:
...
function loadApolloMap(): Map<string, ApolloRecord> {
  const lines = readFileSync('/tmp/apr14-outreach/apollo_results.jsonl', 'utf-8')
    .split('\n').filter(Boolean);
...
async function main() {
  ...
  const outPath = '/tmp/apr14-outreach/final.jsonl';
