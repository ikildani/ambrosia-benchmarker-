...
interface BATNACalculatorProps {
  /** Called whenever the computed BATNA value changes */
  onChange: (batnaUpfrontM: number) => void;
  /** Initial value for the BATNA ($M) */
  initialValue?: number;
}
...
export function BATNACalculator({ onChange, initialValue }: BATNACalculatorProps) {
  ...
  // Auto-compute BATNA from cash runway inputs
  const computedBATNA =
    cashOnHand && burnRate && monthsToInflection
      ? Math.max(0, Number(cashOnHand) - Number(burnRate) * Number(monthsToInflection))
      : null;

  // Feed value back to parent
  useEffect(() => {
  ...
  return (
    <div className="rounded-lg border border-teal-500/30 bg-teal-500/5 p-4">
      ...
      {/* Display computed BATNA */}
      {activeValue != null && (
        <div className="mt-3 flex items-baseline gap-2">
          <span className="text-xs text-slate-500">Computed BATNA:</span>
          <span className="font-mono text-lg font-semibold text-teal-300">
            ${Math.round(activeValue)}M
          </span>
          {!useManual && computedBATNA != null && (
            <span className="text-[11px] text-slate-500">
              = ${cashOnHand}M &minus; ${burnRate}M/mo &times; {monthsToInflection}mo
            </span>
          )}
        </div>
      )}
