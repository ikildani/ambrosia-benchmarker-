...
interface ReportGenerationModalProps {
  ...
  /** Playbook already generated by Results (pre-generated on results load). Skips a 12–40 s fetch. */
  existingPlaybook?: NegotiationPlaybook | null;
  reportId?: string;
  userId?: string;
  userEmail?: string;
...
export default function ReportGenerationModal({
  ...
  const propsRef = useRef({ result, fullInputs, partnerMatches, existingMemo, existingPlaybook, reportId, userId, userEmail, labels, format, onMemoGenerated, onDownloadComplete, onClose, buyerSpecificValuation, buyerSpecificValuations });
  propsRef.current = { result, fullInputs, partnerMatches, existingMemo, existingPlaybook, reportId, userId, userEmail, labels, format, onMemoGenerated, onDownloadComplete, onClose, buyerSpecificValuation, buyerSpecificValuations };
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        if (p.format === 'pdf' && !p.existingPlaybook) {
        ...
        let playbook: NegotiationPlaybook | null = p.existingPlaybook ?? null;
