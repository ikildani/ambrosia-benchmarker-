...
export default function DealTable({ deals, sortBy, sortOrder, onSort, tier }: DealTableProps) {
  const SortHeader = ({ field, children }: { field: string; children: React.ReactNode }) => (
    <th
      ...
      className="px-4 py-3 text-left text-xs font-semibold text-neutral-500 uppercase tracking-wider cursor-pointer hover:text-navy-800 transition-colors"
    >
        ...
        <table className="w-full">
          <thead className="bg-neutral-50 border-b border-neutral-200">
            <tr>
              ...
              <SortHeader field="modality">Modality</SortHeader>
              <SortHeader field="phase_at_signing">Phase</SortHeader>
          ...
          <tbody className="divide-y divide-neutral-100">
            {deals.map((deal, index) => (
              ...
              >
                ...
                <td className="px-4 py-3">
                  <span className="inline-flex px-2 py-0.5 text-xs font-medium bg-teal-100 text-teal-700 rounded-full capitalize">
                ...
                <td className="px-4 py-3 text-sm font-medium text-success-600 whitespace-nowrap">
                  {deal.blurred ? (
                    <span className="inline-flex items-center gap-1 text-neutral-400">
                      <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
