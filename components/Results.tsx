...
export default function Results({ result, tier = 'free', onUpgrade, onBuyReport, reportId, userId, userEmail, inputs, fullInputs, onApplyNewInputs, onPartnerMatchesLoaded }: ResultsProps) {
  ...
  const handleLinkedInShare = () => {
    const text = `${labels.phase} ${labels.modality} deals for ${labels.indication}: Upfront ${formatCurrency(terms.upfront.low)}-${formatCurrency(terms.upfront.high)}, Total value up to ${formatCurrency(terms.totalDealValue.high)}.\n\nBenchmark your deal terms: https://solidus.ambrosiaventures.co/calculator`;
    const url = `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent('https://solidus.ambrosiaventures.co/calculator')}&summary=${encodeURIComponent(text)}`;
    window.open(url, '_blank', 'width=600,height=500');
  };

  const handleXShare = () => {
    const text = `${labels.phase} ${labels.modality} deal for ${labels.indication}: Upfront ${formatCurrency(terms.upfront.low)}-${formatCurrency(terms.upfront.high)}, Total value up to ${formatCurrency(terms.totalDealValue.high)}. solidus.ambrosiaventures.co`;
      ...
      <div className="p-4 sm:p-6 lg:p-8 xl:p-10 bg-gradient-subtle">
        ...
        {fullInputs && (
          <FinancialErrorBoundary fallbackTitle="Deal data unavailable">
            <DealTransparency inputs={fullInputs} tier={tier} onUpgrade={onUpgrade} calculationMedian={terms.upfront.median} peerBenchmark={peerBenchmark} />
          </FinancialErrorBoundary>
        )}
                ...
                {partnerMatches.length > 0 ? (
                  ...
                  <div className="mt-6 p-4 bg-slate-800/30 border border-slate-700/50 rounded-xl">
                    <div className="flex items-center gap-2 text-sm text-slate-400">
                      ...
                      Buyer-Specific Valuation — waiting for partner matches...
                    </div>
                  </div>
                )}
              </div>
        ...
        <div className="mt-6 text-center">
          <p className="text-xs text-slate-500 dark:text-slate-400">
            ...
            {' '}&middot;{' '}{DEAL_STATS.TOTAL_DEALS} curated deals across 12 therapeutic areas{' '}&middot;{' '}Refreshed daily via SEC EDGAR
          </p>
        </div>
