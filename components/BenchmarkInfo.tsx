  ...
  return (
    <>
      ...
      <button
        ...
        className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-gradient-to-r from-teal-50 to-cyan-50 hover:from-teal-100 hover:to-cyan-100 text-teal-700 text-xs font-medium rounded-full border border-teal-200 transition-all duration-200 group"
      >
        <svg className="w-3.5 h-3.5 text-teal-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>
        <span>Data: {metadata.lastUpdated}</span>
        <svg className="w-3 h-3 text-teal-500 group-hover:translate-x-0.5 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[80vh] overflow-hidden animate-fade-in">
            ...
            <div className="overflow-y-auto max-h-[60vh]">
              ...
              <div className="p-6 border-b border-neutral-100">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-bold text-navy-800">{metadata.lastUpdated}</p>
                  </div>
                  <div className="ml-auto text-right">
                    <p className="text-sm text-neutral-500">Next Update</p>
                    <p className="text-lg font-bold text-teal-600">{(metadata as { nextUpdate?: string }).nextUpdate || 'TBD'}</p>
                  </div>
                </div>

                {/* Data Sources */}
                <div className="mt-4 pt-4 border-t border-neutral-100">
                  <p className="text-xs font-semibold text-neutral-500 uppercase tracking-wider mb-2">Data Sources</p>
              ...
              {/* Changelog */}
              <div className="p-6">
                <h4 className="text-xs font-semibold text-neutral-500 uppercase tracking-wider mb-4">Changelog</h4>
                <div className="space-y-4">
                  {changelog.map((entry, idx) => (
                    <div key={idx} className="relative pl-4 border-l-2 border-teal-200">
                      ...
                      <div className="mb-1">
                        <span className="text-sm font-bold text-navy-800">{entry.summary}</span>
                        <span className="ml-2 text-xs text-neutral-500">v{entry.version}</span>
                      </div>
                      <p className="text-xs text-neutral-500 mb-2">{formatDate(entry.date)}</p>
                      <ul className="space-y-1">
                        {entry.changes.map((change, changeIdx) => (
                          <li key={changeIdx} className="text-sm text-neutral-600 flex items-start gap-2">
