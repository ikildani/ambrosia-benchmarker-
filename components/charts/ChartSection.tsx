  ...
  return (
    <div className="mt-6 sm:mt-8">
      <button
        ...
        className="w-full flex items-center justify-between p-4 bg-gradient-to-r from-teal-50 to-cyan-50 dark:from-teal-500/20 dark:to-cyan-500/20 rounded-xl border border-teal-200 dark:border-teal-500/30 hover:border-teal-300 dark:hover:border-teal-500/50 transition-all"
      >
        <div className="flex items-center gap-3">
          ...
          <span className="font-semibold text-teal-800 dark:text-teal-300">Interactive Charts</span>
          {!isPro && (
            <span className="px-2 py-0.5 bg-navy-100 dark:bg-slate-600 text-navy-700 dark:text-slate-200 text-xs font-medium rounded-full">
              Pro
            </span>
          )}
        </div>
      ...
      {isExpanded && (
        <div className="mt-3 bg-white dark:bg-slate-800 rounded-xl border border-neutral-200 dark:border-slate-700 overflow-hidden animate-fade-in">
          {isPro ? (
            <>
              {/* Tabs */}
              <div className="flex border-b border-neutral-200 dark:border-slate-700 overflow-x-auto gap-1 sm:gap-2">
                {tabs.map((tab) => (
                  <button
                    key={tab.id}
                    ...
                    className={`flex items-center gap-1.5 sm:gap-2 px-3 sm:px-6 py-3 text-xs sm:text-sm font-medium transition-colors whitespace-nowrap flex-shrink-0 ${
                      activeTab === tab.id
                        ? 'text-teal-600 dark:text-teal-400 border-b-2 border-teal-500 -mb-px bg-teal-50/50 dark:bg-teal-500/10'
                        : 'text-neutral-600 dark:text-slate-400 hover:text-neutral-800 dark:hover:text-white hover:bg-neutral-50 dark:hover:bg-slate-700/50'
                    }`}
                  >
                    {tab.icon}
                    <span>{tab.label}</span>
            ...
            <div className="p-8 text-center">
              ...
              <p className="text-neutral-600 dark:text-slate-300 mb-4 max-w-md mx-auto">
